...
DeepSeek is the v1.2 provider. This is a thin Python boundary, not LangChain/LangGraph.
Implement against an audited, fixed Hermes commit. Keep the real blocking engine
...
@dataclass(frozen=True)
class RunResult:
    status: Literal["SUCCEEDED", "FAILED", "CANCELLED", "TIMED_OUT", "BUDGET_EXCEEDED"]
...
class HermesAdapter(Protocol):
    def run_stage(
        ...
        """Returns only schema-validated payloads; enforces per-run isolation."""
        ...
